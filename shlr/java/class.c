...
static inline void bn_free(RBinName *bn) {
	if (bn) {
		free (bn->name);
		free (bn->oname);
		free (bn->fname);
...
static inline RBinName *bn_clone(RBinName *bn) {
...
static inline const char *bn_tostring(RBinName *bn) {
...
static RBinJavaCPTypeObj R_BIN_JAVA_NULL_TYPE = { .metas = &R_BIN_JAVA_NULL_TYPE_METAS };
static const ut8 R_BIN_JAVA_CP_METAS_SZ = R_BIN_JAVA_CP_PACKAGE + 1;
...
static RBinJavaCPTypeObj *r_bin_java_find_cp_ref_info(RBinJavaObj *bin, ut16 name_and_type_idx);
static RBinJavaCPTypeObj *r_bin_java_find_cp_ref_info_from_name_and_type(RBinJavaObj *bin, ut16 name_idx, ut16 descriptor_idx);
static ut64 r_bin_java_find_method_offset(RBinJavaObj *bin, const char *method_name);
static ut8 *r_bin_java_get_attr_buf(RBinJavaObj *bin, ut64 sz, const ut64 offset, const ut8 *buf, const ut64 len);
static void r_bin_java_get_class_info_json(RBinJavaObj *bin, PJ *pj);
static char *r_bin_java_get_desc_from_bin_cp_list(RBinJavaObj *bin, ut64 idx);
...
static char *r_bin_java_get_item_desc_from_cp_item_list(RList *cp_list, RBinJavaCPTypeObj *obj, int depth);
static char *r_bin_java_get_item_name_from_bin_cp_list(RBinJavaObj *bin, RBinJavaCPTypeObj *obj);
static char *r_bin_java_get_item_name_from_cp_item_list(RList *cp_list, RBinJavaCPTypeObj *obj, int depth);
static RBinJavaAttrInfo *r_bin_java_get_method_code_attribute(const RBinJavaField *method);
static ut64 r_bin_java_get_method_code_offset(RBinJavaField *fm_type);
static ut64 r_bin_java_get_method_code_size(RBinJavaField *fm_type);
static void r_bin_java_get_method_json_definition(RBinJavaObj *bin, RBinJavaField *fm_type, PJ *pj);
static void r_bin_java_get_method_json_definitions(RBinJavaObj *bin, PJ *pj);
static char *r_bin_java_get_name_from_bin_cp_list(RBinJavaObj *bin, ut64 idx);
static char *r_bin_java_get_name_from_cp_item_list(RList *cp_list, ut64 idx);
static char *r_bin_java_get_utf8_from_bin_cp_list(RBinJavaObj *bin, ut64 idx);
static char *r_bin_java_get_utf8_from_cp_item_list(RList *cp_list, ut64 idx);
static ut32 r_bin_java_get_utf8_len_from_bin_cp_list(RBinJavaObj *bin, ut64 idx);
static ut32 r_bin_java_get_utf8_len_from_cp_item_list(RList *cp_list, ut64 idx);
static ut64 r_bin_java_parse_attrs(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len);
static ut64 r_bin_java_parse_cp_pool(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len);
static ut64 r_bin_java_parse_fields(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len);
static ut64 r_bin_java_parse_interfaces(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len);
static ut64 r_bin_java_parse_methods(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len);
static char *r_bin_java_print_classref_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_double_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_fieldref_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_float_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_integer_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_interfacemethodref_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_invokedynamic_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_long_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_methodhandle_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_methodref_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_methodtype_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_name_and_type_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_null_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_string_cp_stringify(RBinJavaCPTypeObj *obj);
static char *r_bin_java_print_unknown_cp_stringify(RBinJavaCPTypeObj *obj);
...
// Internal CP allocation functions - used only via callback table
static RBinJavaCPTypeObj *class_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *fieldref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *methodref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *ifacemethodref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *nametype_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *string_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *integer_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *float_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *long_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *double_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *utf8_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 offset);
static RBinJavaCPTypeObj *do_nothing_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
...
static RBinJavaCPTypeObj *methodhandle_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
static RBinJavaCPTypeObj *methodtype_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
static RBinJavaCPTypeObj *invokedynamic_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
static RBinJavaCPTypeObj *dynamic_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
static RBinJavaCPTypeObj *module_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
static RBinJavaCPTypeObj *package_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
// Deallocs for type objects
static void cp_default_free(void /*RBinJavaCPTypeObj*/ *obj);
static void r_bin_java_obj_free(void /*RBinJavaCPTypeObj*/ *obj);
static void utf8_info_free(void /*RBinJavaCPTypeObj*/ *obj);
static void do_nothing_free(void /*RBinJavaCPTypeObj*/ *obj);
...
// Forward declaration - actual definition with table comes later
typedef struct r_bin_java_attr_type_info_t RBinJavaAttrTypeInfo;
static const RBinJavaAttrTypeInfo *r_bin_java_get_attr_type_by_name(const char *name);
static RBinJavaCPTypeObj *r_bin_java_get_java_null_cp(void);
...
static RBinJavaCPTypeObj *unknown_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz);
...
static void r_bin_java_element_pair_free(void /*RBinJavaElementValuePair*/ *evp);
...
static void r_bin_java_nest_host_attr_free(void /*RBinJavaAttrInfo*/ *attr);
static void r_bin_java_nest_members_attr_free(void /*RBinJavaAttrInfo*/ *attr);
static void r_bin_java_permitted_subclasses_attr_free(void /*RBinJavaAttrInfo*/ *attr);
static void r_bin_java_record_attr_free(void /*RBinJavaAttrInfo*/ *attr);
...
static void r_bin_java_print_element_pair_summary(RBinJavaElementValuePair *evp);
...
static void r_bin_java_print_nest_host_attr_summary(RBinJavaAttrInfo *attr);
static void r_bin_java_print_nest_members_attr_summary(RBinJavaAttrInfo *attr);
static void r_bin_java_print_permitted_subclasses_attr_summary(RBinJavaAttrInfo *attr);
static void r_bin_java_print_record_attr_summary(RBinJavaAttrInfo *attr);
...
static RBinJavaAttrInfo *r_bin_java_unknown_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_annotation_default_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_enclosing_methods_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_local_variable_type_table_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_signature_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_source_debug_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_bootstrap_methods_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_rtv_annotations_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_rti_annotations_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_rtvp_annotations_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_rtip_annotations_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_code_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_constant_value_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_deprecated_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_exceptions_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_inner_classes_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_line_number_table_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_local_variable_table_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_source_code_file_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_stack_map_table_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_synthetic_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_nest_host_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_nest_members_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_permitted_subclasses_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
static RBinJavaAttrInfo *r_bin_java_record_attr_new(RBinJavaObj *bin, ut8 *buf, ut64 sz, ut64 buf_offset);
...
static ut64 r_bin_java_nest_host_attr_calc_size(RBinJavaAttrInfo *attr);
static ut64 r_bin_java_nest_members_attr_calc_size(RBinJavaAttrInfo *attr);
static ut64 r_bin_java_permitted_subclasses_attr_calc_size(RBinJavaAttrInfo *attr);
static ut64 r_bin_java_record_attr_calc_size(RBinJavaAttrInfo *attr);
static ut64 r_bin_java_bootstrap_method_calc_size(RBinJavaBootStrapMethod *bsm);
static ut64 r_bin_java_element_pair_calc_size(RBinJavaElementValuePair *evp);
static ut64 r_bin_java_element_value_calc_size(RBinJavaElementValue *element_value);

static ut64 unknown_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 class_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 fieldref_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 methodref_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 ifacemethodref_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 nametype_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 string_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 integer_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 float_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 long_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 double_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 utf8_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 do_nothing_calc_size(RBinJavaCPTypeObj *obj);
static ut64 methodhandle_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 methodtype_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 invokedynamic_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 dynamic_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 module_cp_calc_size(RBinJavaCPTypeObj *obj);
static ut64 package_cp_calc_size(RBinJavaCPTypeObj *obj);
static RBinJavaStackMapFrame *r_bin_java_default_stack_frame(void);
...
static RBinJavaAccessFlags CLASS_ACCESS_FLAGS[] = {
	...
	{ "module", R_BIN_JAVA_CLASS_ACC_MODULE, 6 },
	{ "record", R_BIN_JAVA_CLASS_ACC_RECORD, 6 },
...
static RBinJavaCPTypeObjectAllocs R_BIN_ALLOCS_CONSTANTS[] = {
	{ do_nothing_new, do_nothing_free, r_bin_java_print_null_cp_summary, do_nothing_calc_size, r_bin_java_print_null_cp_stringify },
	{ utf8_cp_new, utf8_info_free, r_bin_java_print_utf8_cp_summary, utf8_cp_calc_size, r_bin_java_print_utf8_cp_stringify },
	{ unknown_cp_new, cp_default_free, r_bin_java_print_unknown_cp_summary, unknown_cp_calc_size, r_bin_java_print_unknown_cp_stringify },
	{ integer_cp_new, cp_default_free, r_bin_java_print_integer_cp_summary, integer_cp_calc_size, r_bin_java_print_integer_cp_stringify },
	{ float_cp_new, cp_default_free, r_bin_java_print_float_cp_summary, float_cp_calc_size, r_bin_java_print_float_cp_stringify },
	{ long_cp_new, cp_default_free, r_bin_java_print_long_cp_summary, long_cp_calc_size, r_bin_java_print_long_cp_stringify },
	{ double_cp_new, cp_default_free, r_bin_java_print_double_cp_summary, double_cp_calc_size, r_bin_java_print_double_cp_stringify },
	{ class_cp_new, cp_default_free, r_bin_java_print_classref_cp_summary, class_cp_calc_size, r_bin_java_print_classref_cp_stringify },
	{ string_cp_new, cp_default_free, r_bin_java_print_string_cp_summary, string_cp_calc_size, r_bin_java_print_string_cp_stringify },
	{ fieldref_cp_new, cp_default_free, r_bin_java_print_fieldref_cp_summary, fieldref_cp_calc_size, r_bin_java_print_fieldref_cp_stringify },
	{ methodref_cp_new, cp_default_free, r_bin_java_print_methodref_cp_summary, methodref_cp_calc_size, r_bin_java_print_methodref_cp_stringify },
	{ ifacemethodref_cp_new, cp_default_free, r_bin_java_print_interfacemethodref_cp_summary, ifacemethodref_cp_calc_size, r_bin_java_print_interfacemethodref_cp_stringify },
	{ nametype_cp_new, cp_default_free, r_bin_java_print_name_and_type_cp_summary, nametype_cp_calc_size, r_bin_java_print_name_and_type_cp_stringify },
	{ NULL, NULL, NULL, NULL, NULL },
	{ NULL, NULL, NULL, NULL, NULL },
	{ methodhandle_cp_new, cp_default_free, r_bin_java_print_methodhandle_cp_summary, methodhandle_cp_calc_size, r_bin_java_print_methodhandle_cp_stringify },
	{ methodtype_cp_new, cp_default_free, r_bin_java_print_methodtype_cp_summary, methodtype_cp_calc_size, r_bin_java_print_methodtype_cp_stringify },
	{ dynamic_cp_new, cp_default_free, r_bin_java_print_invokedynamic_cp_summary, dynamic_cp_calc_size, r_bin_java_print_invokedynamic_cp_stringify },
	{ invokedynamic_cp_new, cp_default_free, r_bin_java_print_invokedynamic_cp_summary, invokedynamic_cp_calc_size, r_bin_java_print_invokedynamic_cp_stringify },
	{ module_cp_new, cp_default_free, r_bin_java_print_classref_cp_summary, module_cp_calc_size, r_bin_java_print_classref_cp_stringify },
	{ package_cp_new, cp_default_free, r_bin_java_print_classref_cp_summary, package_cp_calc_size, r_bin_java_print_classref_cp_stringify },
};
static RBinJavaCPTypeMetas R_BIN_JAVA_CP_METAS[] = {
	...
	{ "Unknown", R_BIN_JAVA_CP_UNKNOWN, 0, &R_BIN_ALLOCS_CONSTANTS[2] },
	{ "Unknown", R_BIN_JAVA_CP_UNKNOWN, 0, &R_BIN_ALLOCS_CONSTANTS[2] },
	{ "MethodHandle", R_BIN_JAVA_CP_METHODHANDLE, 4, &R_BIN_ALLOCS_CONSTANTS[15] }, // 1 ref_kind, 2 ref_idx
	{ "MethodType", R_BIN_JAVA_CP_METHODTYPE, 3, &R_BIN_ALLOCS_CONSTANTS[16] }, // 2 descriptor_idx
	{ "Dynamic", R_BIN_JAVA_CP_DYNAMIC, 5, &R_BIN_ALLOCS_CONSTANTS[17] }, // 2 bootstrap_method_attr_idx, 2 name_and_type_idx
	{ "InvokeDynamic", R_BIN_JAVA_CP_INVOKEDYNAMIC, 5, &R_BIN_ALLOCS_CONSTANTS[18] }, // 2 bootstrap_method_attr_idx, 2 name_and_type_idx
	{ "Module", R_BIN_JAVA_CP_MODULE, 3, &R_BIN_ALLOCS_CONSTANTS[19] }, // 2 name_idx
	{ "Package", R_BIN_JAVA_CP_PACKAGE, 3, &R_BIN_ALLOCS_CONSTANTS[20] }, // 2 name_idx
};
// Unified attribute metadata table - combines name, type, and callbacks into single definition
struct r_bin_java_attr_type_info_t {
	const char *name;
	R_BIN_JAVA_ATTR_TYPE type;
...
static const RBinJavaAttrTypeInfo RBIN_JAVA_ATTRS[] = {
	{ "AnnotationDefault", R_BIN_JAVA_ATTR_TYPE_ANNOTATION_DEFAULT_ATTR, r_bin_java_annotation_default_attr_new, r_bin_java_annotation_default_attr_free, r_bin_java_print_annotation_default_attr_summary, r_bin_java_annotation_default_attr_calc_size },
	{ "BootstrapMethods", R_BIN_JAVA_ATTR_TYPE_BOOTSTRAP_METHODS_ATTR, r_bin_java_bootstrap_methods_attr_new, r_bin_java_bootstrap_methods_attr_free, r_bin_java_print_bootstrap_methods_attr_summary, r_bin_java_bootstrap_methods_attr_calc_size },
	{ "Code", R_BIN_JAVA_ATTR_TYPE_CODE_ATTR, r_bin_java_code_attr_new, r_bin_java_code_attr_free, r_bin_java_print_code_attr_summary, r_bin_java_code_attr_calc_size },
	{ "ConstantValue", R_BIN_JAVA_ATTR_TYPE_CONST_VALUE_ATTR, r_bin_java_constant_value_attr_new, r_bin_java_constant_value_attr_free, r_bin_java_print_constant_value_attr_summary, r_bin_java_constant_value_attr_calc_size },
	{ "Deprecated", R_BIN_JAVA_ATTR_TYPE_DEPRECATED_ATTR, r_bin_java_deprecated_attr_new, r_bin_java_deprecated_attr_free, r_bin_java_print_deprecated_attr_summary, r_bin_java_deprecated_attr_calc_size },
	{ "EnclosingMethod", R_BIN_JAVA_ATTR_TYPE_ENCLOSING_METHOD_ATTR, r_bin_java_enclosing_methods_attr_new, r_bin_java_enclosing_methods_attr_free, r_bin_java_print_enclosing_methods_attr_summary, r_bin_java_enclosing_methods_attr_calc_size },
	{ "Exceptions", R_BIN_JAVA_ATTR_TYPE_EXCEPTIONS_ATTR, r_bin_java_exceptions_attr_new, r_bin_java_exceptions_attr_free, r_bin_java_print_exceptions_attr_summary, r_bin_java_exceptions_attr_calc_size },
	{ "InnerClasses", R_BIN_JAVA_ATTR_TYPE_INNER_CLASSES_ATTR, r_bin_java_inner_classes_attr_new, r_bin_java_inner_classes_attr_free, r_bin_java_print_inner_classes_attr_summary, r_bin_java_inner_classes_attr_calc_size },
	{ "LineNumberTable", R_BIN_JAVA_ATTR_TYPE_LINE_NUMBER_TABLE_ATTR, r_bin_java_line_number_table_attr_new, r_bin_java_line_number_table_attr_free, r_bin_java_print_line_number_table_attr_summary, r_bin_java_line_number_table_attr_calc_size },
	{ "LocalVariableTable", R_BIN_JAVA_ATTR_TYPE_LOCAL_VARIABLE_TABLE_ATTR, r_bin_java_local_variable_table_attr_new, r_bin_java_local_variable_table_attr_free, r_bin_java_print_local_variable_table_attr_summary, r_bin_java_local_variable_table_attr_calc_size },
	{ "LocalVariableTypeTable", R_BIN_JAVA_ATTR_TYPE_LOCAL_VARIABLE_TYPE_TABLE_ATTR, r_bin_java_local_variable_type_table_attr_new, r_bin_java_local_variable_type_table_attr_free, r_bin_java_print_local_variable_type_table_attr_summary, r_bin_java_local_variable_type_table_attr_calc_size },
	{ "RuntimeInvisibleAnnotations", R_BIN_JAVA_ATTR_TYPE_RUNTIME_INVISIBLE_ANNOTATION_ATTR, r_bin_java_rti_annotations_attr_new, r_bin_java_rti_annotations_attr_free, r_bin_java_print_rti_annotations_attr_summary, r_bin_java_rti_annotations_attr_calc_size },
	{ "RuntimeInvisibleParameterAnnotations", R_BIN_JAVA_ATTR_TYPE_RUNTIME_INVISIBLE_PARAMETER_ANNOTATION_ATTR, r_bin_java_rtip_annotations_attr_new, r_bin_java_rtip_annotations_attr_free, r_bin_java_print_rtip_annotations_attr_summary, r_bin_java_rtip_annotations_attr_calc_size },
	{ "RuntimeVisibleAnnotations", R_BIN_JAVA_ATTR_TYPE_RUNTIME_VISIBLE_ANNOTATION_ATTR, r_bin_java_rtv_annotations_attr_new, r_bin_java_rtv_annotations_attr_free, r_bin_java_print_rtv_annotations_attr_summary, r_bin_java_rtv_annotations_attr_calc_size },
	{ "RuntimeVisibleParameterAnnotations", R_BIN_JAVA_ATTR_TYPE_RUNTIME_VISIBLE_PARAMETER_ANNOTATION_ATTR, r_bin_java_rtvp_annotations_attr_new, r_bin_java_rtvp_annotations_attr_free, r_bin_java_print_rtvp_annotations_attr_summary, r_bin_java_rtvp_annotations_attr_calc_size },
	{ "Signature", R_BIN_JAVA_ATTR_TYPE_SIGNATURE_ATTR, r_bin_java_signature_attr_new, r_bin_java_signature_attr_free, r_bin_java_print_signature_attr_summary, r_bin_java_signature_attr_calc_size },
	{ "SourceDebugExtension", R_BIN_JAVA_ATTR_TYPE_SOURCE_DEBUG_EXTENTSION_ATTR, r_bin_java_source_debug_attr_new, r_bin_java_source_debug_attr_free, r_bin_java_print_source_debug_attr_summary, r_bin_java_source_debug_attr_calc_size },
	{ "SourceFile", R_BIN_JAVA_ATTR_TYPE_SOURCE_FILE_ATTR, r_bin_java_source_code_file_attr_new, r_bin_java_source_code_file_attr_free, r_bin_java_print_source_code_file_attr_summary, r_bin_java_source_code_file_attr_calc_size },
	{ "StackMapTable", R_BIN_JAVA_ATTR_TYPE_STACK_MAP_TABLE_ATTR, r_bin_java_stack_map_table_attr_new, r_bin_java_stack_map_table_attr_free, r_bin_java_print_stack_map_table_attr_summary, r_bin_java_stack_map_table_attr_calc_size },
	{ "Synthetic", R_BIN_JAVA_ATTR_TYPE_SYNTHETIC_ATTR, r_bin_java_synthetic_attr_new, r_bin_java_synthetic_attr_free, r_bin_java_print_synthetic_attr_summary, r_bin_java_synthetic_attr_calc_size },
	{ "NestHost", R_BIN_JAVA_ATTR_TYPE_NEST_HOST_ATTR, r_bin_java_nest_host_attr_new, r_bin_java_nest_host_attr_free, r_bin_java_print_nest_host_attr_summary, r_bin_java_nest_host_attr_calc_size },
	{ "NestMembers", R_BIN_JAVA_ATTR_TYPE_NEST_MEMBERS_ATTR, r_bin_java_nest_members_attr_new, r_bin_java_nest_members_attr_free, r_bin_java_print_nest_members_attr_summary, r_bin_java_nest_members_attr_calc_size },
	{ "PermittedSubclasses", R_BIN_JAVA_ATTR_TYPE_PERMITTED_SUBCLASSES_ATTR, r_bin_java_permitted_subclasses_attr_new, r_bin_java_permitted_subclasses_attr_free, r_bin_java_print_permitted_subclasses_attr_summary, r_bin_java_permitted_subclasses_attr_calc_size },
	{ "Record", R_BIN_JAVA_ATTR_TYPE_RECORD_ATTR, r_bin_java_record_attr_new, r_bin_java_record_attr_free, r_bin_java_print_record_attr_summary, r_bin_java_record_attr_calc_size },
	{ "Unknown", R_BIN_JAVA_ATTR_TYPE_UNKNOWN_ATTR, r_bin_java_unknown_attr_new, r_bin_java_unknown_attr_free, r_bin_java_print_unknown_attr_summary, r_bin_java_unknown_attr_calc_size }
};
#define RBIN_JAVA_ATTRS_COUNT (sizeof (RBIN_JAVA_ATTRS) / sizeof (RBIN_JAVA_ATTRS[0]))
#define RBIN_JAVA_ATTRS_METAS RBIN_JAVA_ATTRS

static bool r_bin_java_is_old_format(RBinJavaObj *bin) {
...
static void r_bin_java_reset_bin_info(RBinJavaObj *bin) {
	...
	bin->imports_list = r_list_newf (bimport_free);
...
static char *r_bin_java_unmangle_method(const char *flags, const char *name, const char *params, const char *r_value) {
	...
	char *str = NULL, *rval = NULL;
	if (!extract_type_value (r_value, &rval)) {
	...
	char *fval = strdup (r_str_get (flags));
	RStrBuf *sb = r_strbuf_new ("");
...
static void r_bin_java_get_class_info_json(RBinJavaObj *bin, PJ *pj) {
	...
	if (klass) {
		...
		RListIter *attr_iter;
		r_list_foreach (bin->attrs_list, attr_iter, attr) {
			if (attr && attr->type == R_BIN_JAVA_ATTR_TYPE_PERMITTED_SUBCLASSES_ATTR) {
				is_sealed = true;
				break;
			}
		}
		pj_kb (pj, "is_sealed", is_sealed);
		pj_ks (pj, "name", bn_tostring (klass->name));
		if (klass->super) {
			RBinName *bn;
			RListIter *iter;
			pj_ka (pj, "super");
...
static char *r_bin_java_create_method_fq_str(const char *klass, const char *name, const char *signature) {
	if (!klass) {
		klass = "null_class";
	}
	if (!name) {
...
static char *r_bin_java_create_field_fq_str(const char *klass, const char *name, const char *signature) {
	if (!klass) {
		klass = "null_class";
	}
	if (!name) {
...
static void r_bin_java_get_fm_type_definition_json(RBinJavaObj *bin, RBinJavaField *fm_type, PJ *pj, bool is_method) {
	...
	pj_kb (pj, "is_method", is_method);
	pj_kb (pj, "is_native", (flags & R_BIN_JAVA_METHOD_ACC_NATIVE) != 0);
	pj_kb (pj, "is_synthetic", (flags & R_BIN_JAVA_METHOD_ACC_SYNTHETIC) != 0);
	pj_kb (pj, "is_private", (flags & R_BIN_JAVA_METHOD_ACC_PRIVATE) != 0);
	pj_kb (pj, "is_public", (flags & R_BIN_JAVA_METHOD_ACC_PUBLIC) != 0);
	pj_kb (pj, "is_static", (flags & R_BIN_JAVA_METHOD_ACC_STATIC) != 0);
	pj_kb (pj, "is_protected", (flags & R_BIN_JAVA_METHOD_ACC_PROTECTED) != 0);
	pj_kb (pj, "is_super", (flags & R_BIN_JAVA_CLASS_ACC_SUPER) != 0);
	...
	char *fq_name = (is_method)
		? r_bin_java_create_method_fq_str (fm_type->class_name, fm_type->name, fm_type->descriptor)
		: r_bin_java_create_field_fq_str (fm_type->class_name, fm_type->name, fm_type->descriptor);
...
static char *retrieve_access_string(ut32 flags, const RBinJavaAccessFlags *access_flags) {
	RStrBuf *sb = r_strbuf_new ("");
	int i;
	for (i = 0; access_flags[i].str; i++) {
...
static RBinJavaCPTypeObj *r_bin_java_get_java_null_cp(void) {
	if (!R_BIN_JAVA_NULL_TYPE.metas->type_info) {
...
static ut8 r_bin_java_quick_check(ut8 expected_tag, ut8 actual_tag, ut32 actual_len, const char *name) {
	...
	if (actual_len < R_BIN_JAVA_CP_METAS[expected_tag].len) {
		R_LOG_WARN ("Unable to parse '%d' expected sz=0x%02x got 0x%02x for %s",
			actual_tag,
			R_BIN_JAVA_CP_METAS[expected_tag].len,
			actual_len,
			name);
		return 2;
	}
...
static double r_bin_java_raw_to_double(const ut8 *raw, ut64 offset) {
...
static RBinJavaField *r_bin_java_read_next_method(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len) {
	...
	R_LOG_DEBUG ("Method name_idx: %d, which is: ord: %d, name: %s, value: %s",
		idx,
		item->metas->ord,
		((RBinJavaCPTypeMetas *)item->metas->type_info)->name,
		method->name);
	if (!method->name) {
	...
	R_LOG_DEBUG ("Method descriptor_idx: %d, which is: ord: %d, name: %s, value: %s", idx, item->metas->ord, ((RBinJavaCPTypeMetas *)item->metas->type_info)->name, method->descriptor);
...
static RBinJavaCPTypeObj *r_bin_java_clone_cp_item(RBinJavaCPTypeObj *obj) {
	...
	RBinJavaCPTypeObj *clone_obj = r_mem_dup (obj, sizeof (RBinJavaCPTypeObj));
	clone_obj->metas = R_NEW0 (RBinJavaMetaInfo);
...
static RBinJavaCPTypeObj *r_bin_java_read_next_constant_pool_item(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, ut64 len) {
	...
	// Validate buf_sz before allocation
	if (buf_sz > 65536) {
		R_LOG_ERROR ("Invalid buffer size: %zu", buf_sz);
		return NULL;
	}
...
static char *r_bin_java_get_item_name_from_cp_item_list(RList *cp_list, RBinJavaCPTypeObj *obj, int depth) {
	...
	default:
		return NULL;
	case 0:
		R_LOG_DEBUG ("Invalid 0 tag in the constant pool");
		return NULL;
	}
	return NULL;
}
...
static RBinJavaAttrInfo *r_bin_java_default_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	const RBinJavaAttrTypeInfo *type_info = r_bin_java_get_attr_type_by_name (attr->name);
	attr->metas->ord = (bin->attr_idx++);
	attr->metas->type_info = (void *)type_info;
	// R_LOG_DEBUG ("Addrs for type_info [tag=%d]: 0x%08"PFMT64x, type_val, &attr->metas->type_info);
	return attr;
}

static const RBinJavaAttrTypeInfo *r_bin_java_get_attr_type_by_name(const char *name) {
	size_t i;
	for (i = 0; i < RBIN_JAVA_ATTRS_COUNT; i++) {
		if (!strcmp (name, RBIN_JAVA_ATTRS[i].name)) {
			return &RBIN_JAVA_ATTRS[i];
		}
	}
	return &RBIN_JAVA_ATTRS[RBIN_JAVA_ATTRS_COUNT - 1]; // Unknown
}

static RBinJavaAttrInfo *r_bin_java_read_next_attr(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 buf_len) {
	const ut8 *a_buf = offset + buf;
	ut8 attr_idx_len = 6;
	if (offset + 6 > buf_len) {
...
static RBinJavaAttrInfo *r_bin_java_read_next_attr_from_buffer(RBinJavaObj *bin, ut8 *buffer, st64 sz, st64 buf_offset) {
	...
	R_LOG_DEBUG ("r_bin_java_read_next_attr: name_idx = %d is %s", name_idx, name);
	const RBinJavaAttrTypeInfo *type_info = r_bin_java_get_attr_type_by_name (name);
	if (type_info) {
		...
		if ((attr = type_info->new_obj (bin, buffer, nsz + 6, buf_offset))) {
			attr->metas->ord = (bin->attr_idx++);
		}
	} else {
...
static ut64 r_bin_java_read_class_file2(RBinJavaObj *bin, const ut64 offset, const ut8 *obuf, ut64 len) {
	...
	R_LOG_DEBUG ("0x%" PFMT64x " Offset before reading the cf2 structure", offset);
	/*
...
static ut64 r_bin_java_parse_interfaces(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len) {
	...
	adv += 2;
	R_LOG_DEBUG ("Interfaces count: %d", bin->interfaces_count);
...
static ut64 r_bin_java_parse_attrs(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len) {
	...
	adv += 2;
	// Validate attrs_count to prevent excessive looping
	const ut16 MAX_ATTRS_COUNT = 4096;
	if (bin->attrs_count > MAX_ATTRS_COUNT) {
		R_LOG_ERROR ("Invalid attributes count: %d", bin->attrs_count);
		return UT64_MAX;
	}
...
static ut64 r_bin_java_parse_methods(RBinJavaObj *bin, const ut64 offset, const ut8 *buf, const ut64 len) {
	...
	for (i = 0; i < bin->methods_count; i++, bin->method_idx++) {
		...
		if (!strcmp ((const char *)method->name, "main")) {
			...
			R_LOG_DEBUG ("Found an init function");
		...
		// r_bin_java_print_field_summary (method);
	}
...
static RBinSymbol *r_bin_java_create_new_symbol_from_fm_type_meta(RBinJavaField *fm_type, ut64 baddr) {
	if (!fm_type || !fm_type->field_ref_cp_obj || fm_type->field_ref_cp_obj == &R_BIN_JAVA_NULL_TYPE) {
		return NULL;
	}
	RBinSymbol *sym = R_NEW0 (RBinSymbol);
...
#if 0
static RList * r_bin_java_get_interface_classes(RBinJavaObj * bin) {
...
R_API RList *r_bin_java_get_classes(RBinJavaObj *bin) {
	RList *classes = r_list_newf ((void *)bclass_free);
	RListIter *iter;
	RBinJavaCPTypeObj *cp_obj = NULL;
	...
	if (R_STR_ISNOTEMPTY (n)) {
		k->super = r_list_newf ((void *)bn_free);
		r_list_append (k->super, bn_new (n));
	}
	free (n);
	k->index = (idx++);
...
static RBinSymbol *r_bin_java_create_new_symbol_from_cp_idx(RBinJavaObj *bin, ut32 cp_idx, ut64 baddr);

static RBinSymbol *r_bin_java_create_new_symbol_from_invoke_dynamic(RBinJavaObj *bin, RBinJavaCPTypeObj *obj, ut64 baddr) {
	if (!obj || (obj->tag != R_BIN_JAVA_CP_INVOKEDYNAMIC)) {
		return NULL;
	}
	return r_bin_java_create_new_symbol_from_cp_idx (bin, obj->info.cp_invoke_dynamic.name_and_type_index, baddr);
}

static RBinSymbol *r_bin_java_create_new_symbol_from_cp_idx(RBinJavaObj *bin, ut32 cp_idx, ut64 baddr) {
	RBinSymbol *sym = NULL;
	RBinJavaCPTypeObj *obj = r_bin_java_get_item_from_bin_cp_list (bin, cp_idx);
	if (obj) {
		switch (obj->tag) {
		case R_BIN_JAVA_CP_METHODREF:
		case R_BIN_JAVA_CP_FIELDREF:
		case R_BIN_JAVA_CP_INTERFACEMETHOD_REF:
			sym = r_bin_java_create_new_symbol_from_ref (bin, obj, baddr);
			break;
		case R_BIN_JAVA_CP_INVOKEDYNAMIC:
			sym = r_bin_java_create_new_symbol_from_invoke_dynamic (bin, obj, baddr);
			break;
		default:
			break;
		}
...
static void r_bin_java_attribute_free(void /*RBinJavaAttrInfo*/ *a) {
	...
	if (attr) {
		...
		if (attr && attr->metas && attr->metas->type_info) {
			const RBinJavaAttrTypeInfo *ti = attr->metas->type_info;
			if (ti && ti->delete_obj) {
				ti->delete_obj (attr);
			}
		}
...
static ut64 r_bin_java_attr_calc_size(RBinJavaAttrInfo *attr) {
	return attr? ((const RBinJavaAttrTypeInfo *)attr->metas->type_info)->calc_size (attr): 0;
}
...
static RBinJavaAttrInfo *r_bin_java_unknown_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
...
static RBinJavaAttrInfo *r_bin_java_code_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	for (k = 0; k < attr->info.code_attr.attributes_count; k++) {
		...
		if (_attr->type == R_BIN_JAVA_ATTR_TYPE_LOCAL_VARIABLE_TABLE_ATTR) {
			...
			attr->info.code_attr.implicit_frame = r_bin_java_build_stack_frame_from_local_variable_table (bin, _attr);
...
static RBinJavaAttrInfo *r_bin_java_constant_value_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	// // r_bin_java_print_constant_value_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_deprecated_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	if (attr) {
		attr->type = R_BIN_JAVA_ATTR_TYPE_DEPRECATED_ATTR;
		attr->size = 6;
	}
	// // r_bin_java_print_deprecated_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_signature_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	...
	attr->size = offset;
	// // r_bin_java_print_source_code_file_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_enclosing_methods_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut64 offset = 6;
	if (sz < 8) {
		return NULL;
	}
...
static RBinJavaAttrInfo *r_bin_java_exceptions_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut32 i = 0, offset = 0;
	ut64 size;
	if (sz < 8) {
		return NULL;
	...
	offset += 2;
	// Validate bounds
	if (offset + attr->info.exceptions_attr.number_of_exceptions * 2 > sz) {
		R_LOG_ERROR ("Invalid exceptions count: %d", attr->info.exceptions_attr.number_of_exceptions);
		free (attr);
		return NULL;
	}
	...
	attr->size = offset;
	// // r_bin_java_print_exceptions_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_inner_classes_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	for (i = 0; i < attr->info.inner_classes_attr.number_of_classes; i++) {
		...
		// eprintf ("r_bin_java_inner_classes_attr: Inner class name %d is %s.\n", icattr->inner_name_idx, icattr->name);
		r_list_append (attr->info.inner_classes_attr.classes, (void *)icattr);
	}
	attr->size = offset;
	// // r_bin_java_print_inner_classes_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_line_number_table_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	// Validate bounds
	if (offset + linenum_len * 4 > sz) {
		R_LOG_ERROR ("Invalid line number table length: %d", linenum_len);
...
static RBinJavaAttrInfo *r_bin_java_source_debug_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut64 offset = 6;
	if (sz < 8) {
		return NULL;
	}
...
static RBinJavaAttrInfo *r_bin_java_local_variable_table_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	RBinJavaLocalVariableAttribute *lvattr;
	...
	// Validate bounds
	if (offset + attr->info.local_variable_table_attr.table_length * 10 > sz) {
		R_LOG_ERROR ("Invalid local variable table length: %d", attr->info.local_variable_table_attr.table_length);
		r_bin_java_attribute_free (attr);
		return NULL;
	}
	for (i = 0; i < attr->info.local_variable_table_attr.table_length; i++) {
		...
		lvattr->name = r_bin_java_get_utf8_from_bin_cp_list (bin, lvattr->name_idx);
		lvattr->size = 10;
		if (!lvattr->name) {
	...
	attr->size = offset;
	// // r_bin_java_print_local_variable_table_attr_summary (attr);
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_local_variable_type_table_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaLocalVariableTypeAttribute *lvattr;
	...
	// Validate bounds
	if (offset + attr->info.local_variable_type_table_attr.table_length * 10 > sz) {
		R_LOG_ERROR ("Invalid local variable type table length: %d", attr->info.local_variable_type_table_attr.table_length);
		r_bin_java_attribute_free (attr);
		return NULL;
	}
	...
	// // r_bin_java_print_local_variable_type_table_attr_summary (attr);
	attr->size = offset;
	return attr;
}
...
static RBinJavaAttrInfo *r_bin_java_synthetic_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
...
// NestHost attribute (Java 11+)
static RBinJavaAttrInfo *r_bin_java_nest_host_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	...
	attr->type = R_BIN_JAVA_ATTR_TYPE_NEST_HOST_ATTR;
	ut64 offset = 6;
	attr->info.nest_host_attr.host_class_idx = R_BIN_JAVA_USHORT (buffer, offset);
	offset += 2;
	attr->info.nest_host_attr.host_class_name = r_bin_java_get_name_from_bin_cp_list (bin, attr->info.nest_host_attr.host_class_idx);
	attr->size = offset;
	return attr;
}

static void r_bin_java_nest_host_attr_free(void *a) {
	RBinJavaAttrInfo *attr = a;
	if (attr) {
		free (attr->info.nest_host_attr.host_class_name);
		free (attr->name);
		free (attr->metas);
		free (attr);
	}
...
static void r_bin_java_print_nest_host_attr_summary(RBinJavaAttrInfo *attr) {
	...
	printf ("NestHost Attribute Information:\n");
	printf ("  Attribute Offset: 0x%08" PFMT64x "\n", attr->file_offset);
	printf ("  Attribute Name Index: %d (%s)\n", attr->name_idx, attr->name);
	printf ("  Attribute Length: %d\n", attr->length);
	printf ("  Host Class Index: %d\n", attr->info.nest_host_attr.host_class_idx);
	printf ("  Host Class Name: %s\n", attr->info.nest_host_attr.host_class_name? attr->info.nest_host_attr.host_class_name: "NULL");
}

static ut64 r_bin_java_nest_host_attr_calc_size(RBinJavaAttrInfo *attr) {
	return 8; // 2 name_idx + 4 length + 2 host_class_idx
}

// NestMembers attribute (Java 11+)
static RBinJavaAttrInfo *r_bin_java_nest_members_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	...
	attr->type = R_BIN_JAVA_ATTR_TYPE_NEST_MEMBERS_ATTR;
	ut64 offset = 6;
	attr->info.nest_members_attr.number_of_classes = R_BIN_JAVA_USHORT (buffer, offset);
	offset += 2;
	...
	if (num > 0 && num < UT16_MAX / 2 && offset + num * 2 <= sz) {
		...
		ut32 i;
		for (i = 0; i < num && offset + 2 <= sz; i++) {
			attr->info.nest_members_attr.classes[i] = R_BIN_JAVA_USHORT (buffer, offset);
			char *name = r_bin_java_get_name_from_bin_cp_list (bin, attr->info.nest_members_attr.classes[i]);
			r_list_append (attr->info.nest_members_attr.class_names, name? name: strdup ("NULL"));
			offset += 2;
		}
	}
	attr->size = offset;
...
static void r_bin_java_nest_members_attr_free(void *a) {
	RBinJavaAttrInfo *attr = a;
	if (attr) {
		free (attr->info.nest_members_attr.classes);
		r_list_free (attr->info.nest_members_attr.class_names);
		free (attr->name);
		free (attr->metas);
		free (attr);
	}
...
static void r_bin_java_print_nest_members_attr_summary(RBinJavaAttrInfo *attr) {
	...
	printf ("NestMembers Attribute Information:\n");
	printf ("  Attribute Offset: 0x%08" PFMT64x "\n", attr->file_offset);
	printf ("  Attribute Name Index: %d (%s)\n", attr->name_idx, attr->name);
	printf ("  Attribute Length: %d\n", attr->length);
	printf ("  Number of Classes: %d\n", attr->info.nest_members_attr.number_of_classes);
	RListIter *iter;
	char *name;
	int i = 0;
	r_list_foreach (attr->info.nest_members_attr.class_names, iter, name) {
		printf ("    Class[%d]: %s\n", i++, name? name: "NULL");
	}
}
...
static ut64 r_bin_java_nest_members_attr_calc_size(RBinJavaAttrInfo *attr) {
	...
	return 8 + attr->info.nest_members_attr.number_of_classes * 2;
}

// PermittedSubclasses attribute (Java 17+ - sealed classes)
static RBinJavaAttrInfo *r_bin_java_permitted_subclasses_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	...
	attr->type = R_BIN_JAVA_ATTR_TYPE_PERMITTED_SUBCLASSES_ATTR;
	ut64 offset = 6;
	attr->info.permitted_subclasses_attr.number_of_classes = R_BIN_JAVA_USHORT (buffer, offset);
	offset += 2;
	...
	if (num > 0 && num < UT16_MAX / 2 && offset + num * 2 <= sz) {
		...
		ut32 i;
		for (i = 0; i < num && offset + 2 <= sz; i++) {
			attr->info.permitted_subclasses_attr.classes[i] = R_BIN_JAVA_USHORT (buffer, offset);
			char *name = r_bin_java_get_name_from_bin_cp_list (bin, attr->info.permitted_subclasses_attr.classes[i]);
			r_list_append (attr->info.permitted_subclasses_attr.class_names, name? name: strdup ("NULL"));
			offset += 2;
		}
	}
	attr->size = offset;
...
static void r_bin_java_permitted_subclasses_attr_free(void *a) {
	RBinJavaAttrInfo *attr = a;
	if (attr) {
		free (attr->info.permitted_subclasses_attr.classes);
		r_list_free (attr->info.permitted_subclasses_attr.class_names);
		free (attr->name);
		free (attr->metas);
		free (attr);
	}
...
static void r_bin_java_print_permitted_subclasses_attr_summary(RBinJavaAttrInfo *attr) {
	...
	printf ("PermittedSubclasses Attribute Information:\n");
	printf ("  Attribute Offset: 0x%08" PFMT64x "\n", attr->file_offset);
	printf ("  Attribute Name Index: %d (%s)\n", attr->name_idx, attr->name);
	printf ("  Attribute Length: %d\n", attr->length);
	printf ("  Number of Classes: %d\n", attr->info.permitted_subclasses_attr.number_of_classes);
	RListIter *iter;
	char *name;
	int i = 0;
	r_list_foreach (attr->info.permitted_subclasses_attr.class_names, iter, name) {
		printf ("    Permitted Class[%d]: %s\n", i++, name? name: "NULL");
	}
}
...
static ut64 r_bin_java_permitted_subclasses_attr_calc_size(RBinJavaAttrInfo *attr) {
	...
	return 8 + attr->info.permitted_subclasses_attr.number_of_classes * 2;
}

// Record attribute (Java 16+)
static void r_bin_java_record_component_free(void *c) {
	RBinJavaRecordComponent *comp = c;
	if (comp) {
		free (comp->name);
		free (comp->descriptor);
		r_list_free (comp->attributes);
		free (comp);
	}
}

static RBinJavaAttrInfo *r_bin_java_record_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 8) {
		return NULL;
	}
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	...
	attr->type = R_BIN_JAVA_ATTR_TYPE_RECORD_ATTR;
	ut64 offset = 6;
	attr->info.record_attr.components_count = R_BIN_JAVA_USHORT (buffer, offset);
	offset += 2;
	...
	for (i = 0; i < num && offset + 6 <= sz; i++) {
		...
		comp->name_idx = R_BIN_JAVA_USHORT (buffer, offset);
		offset += 2;
		comp->descriptor_idx = R_BIN_JAVA_USHORT (buffer, offset);
		offset += 2;
		comp->name = r_bin_java_get_utf8_from_bin_cp_list (bin, comp->name_idx);
		comp->descriptor = r_bin_java_get_utf8_from_bin_cp_list (bin, comp->descriptor_idx);
		comp->attributes_count = R_BIN_JAVA_USHORT (buffer, offset);
		offset += 2;
		...
		r_list_append (attr->info.record_attr.components, comp);
	}
	attr->size = offset;
	return attr;
}

static void r_bin_java_record_attr_free(void *a) {
	RBinJavaAttrInfo *attr = a;
	if (attr) {
		r_list_free (attr->info.record_attr.components);
		free (attr->name);
		free (attr->metas);
		free (attr);
	}
...
static void r_bin_java_print_record_attr_summary(RBinJavaAttrInfo *attr) {
	...
	printf ("Record Attribute Information:\n");
	printf ("  Attribute Offset: 0x%08" PFMT64x "\n", attr->file_offset);
	printf ("  Attribute Name Index: %d (%s)\n", attr->name_idx, attr->name);
	printf ("  Attribute Length: %d\n", attr->length);
	printf ("  Components Count: %d\n", attr->info.record_attr.components_count);
	RListIter *iter;
	RBinJavaRecordComponent *comp;
	int i = 0;
	r_list_foreach (attr->info.record_attr.components, iter, comp) {
		printf ("    Component[%d]: %s %s\n", i++, comp->descriptor? comp->descriptor: "?", comp->name? comp->name: "?");
	}
}
...
static ut64 r_bin_java_record_attr_calc_size(RBinJavaAttrInfo *attr) {
	...
	ut64 size = 8; // header + components_count
	RListIter *iter;
	RBinJavaRecordComponent *comp;
	r_list_foreach (attr->info.record_attr.components, iter, comp) {
		size += 6; // name_idx + descriptor_idx + attributes_count
		RListIter *aiter;
		RBinJavaAttrInfo *cattr;
		r_list_foreach (comp->attributes, aiter, cattr) {
			size += cattr->size;
		}
	}
	return size;
...
static RBinJavaInterfaceInfo *r_bin_java_interface_new(RBinJavaObj *bin, const ut8 *buffer, ut64 sz) {
	// eprintf ("Parsing RBinJavaInterfaceInfo\n");
...
static RBinJavaVerificationObj *r_bin_java_verification_info_from_type(RBinJavaObj *bin, R_BIN_JAVA_STACKMAP_TYPE type, ut32 value) {
...
static RBinJavaVerificationObj *r_bin_java_read_from_buffer_verification_info_new(ut8 *buffer, ut64 sz, ut64 buf_offset) {
	if (sz < 1) {
		return NULL;
	}
	ut64 offset = 0;
...
static RBinJavaStackMapFrameMetas *r_bin_java_determine_stack_frame_type(ut8 tag) {
	ut8 type_value = 0;
	if (tag < 64) {
...
static ut64 r_bin_java_stack_map_frame_calc_size(RBinJavaStackMapFrame *sf) {
	ut64 size = 0;
...
static RBinJavaStackMapFrame *r_bin_java_stack_map_frame_new(ut8 *buffer, ut64 sz, RBinJavaStackMapFrame *p_frame, ut64 buf_offset) {
	...
	case R_BIN_JAVA_STACK_FRAME_SAME_LOCALS_1:
		...
		// eprintf ("r_bin_java_stack_map_frame_new: Parsed R_BIN_JAVA_STACK_FRAME_SAME_LOCALS_1.\n");
		if (se) {
			offset += se->size;
		} else {
	...
	case R_BIN_JAVA_STACK_FRAME_CHOP:
		...
		// eprintf ("r_bin_java_stack_map_frame_new: Parsing R_BIN_JAVA_STACK_FRAME_CHOP.\n");
...
static RBinJavaStackMapFrame *r_bin_java_build_stack_frame_from_local_variable_table(RBinJavaObj *bin, RBinJavaAttrInfo *attr) {
	...
	if (value_cnt != attr->info.local_variable_table_attr.table_length) {
		R_LOG_DEBUG ("r_bin_java_build_stack_frame_from_local_variable_table: Number of locals not accurate. Expected %d but got %d", attr->info.local_variable_table_attr.table_length, value_cnt);
	}
	return sf;
}
...
static RBinJavaAttrInfo *r_bin_java_stack_map_table_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut32 i = 0;
	ut64 offset = 0;
	if (sz < 8) {
		return NULL;
	...
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
	offset += 6;
	R_LOG_DEBUG ("r_bin_java_stack_map_table_attr_new: New stack map allocated");
	if (!attr) {
		return NULL;
	}
...
#if 0
	R_LOG_DEBUG ("r_bin_java_stack_map_table_attr_new: Processing stack map, summary is:");
	r_bin_java_print_stack_map_table_attr_summary (attr);
#endif
	for (i = 0; i < attr->info.stack_map_table_attr.number_of_entries; i++) {
		...
		// Check bin first to avoid null pointer dereference
		if (bin && bin->current_code_attr && stack_frame == NULL) {
...
static RBinJavaCPTypeObj *class_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *fieldref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
	int quick_check = 0;
...
static RBinJavaCPTypeObj *methodref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
	int quick_check = 0;
...
static RBinJavaCPTypeObj *ifacemethodref_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *string_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *integer_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *float_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *long_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *double_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *nametype_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	RBinJavaCPTypeObj *obj = R_NEW0 (RBinJavaCPTypeObj);
	ut8 tag = buffer[0];
...
static RBinJavaCPTypeObj *dynamic_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
	int quick_check = r_bin_java_quick_check (R_BIN_JAVA_CP_DYNAMIC, tag, sz, "RBinJavaCPTypeDynamic");
	if (quick_check > 0) {
		return NULL;
	}
	RBinJavaCPTypeObj *obj = R_NEW0 (RBinJavaCPTypeObj);
	...
	obj->name = strdup ((const char *)R_BIN_JAVA_CP_METAS[tag].name);
	obj->tag = tag;
	obj->info.cp_dynamic.bootstrap_method_attr_index = R_BIN_JAVA_USHORT (buffer, 1);
	obj->info.cp_dynamic.name_and_type_index = R_BIN_JAVA_USHORT (buffer, 3);
	return obj;
}

static ut64 dynamic_cp_calc_size(RBinJavaCPTypeObj *obj) {
	return 5; // 1 tag + 2 bootstrap_method_attr_index + 2 name_and_type_index
}

static RBinJavaCPTypeObj *module_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
	int quick_check = r_bin_java_quick_check (R_BIN_JAVA_CP_MODULE, tag, sz, "RBinJavaCPTypeModule");
	if (quick_check > 0) {
		return NULL;
	}
	RBinJavaCPTypeObj *obj = R_NEW0 (RBinJavaCPTypeObj);
	...
	obj->name = strdup ((const char *)R_BIN_JAVA_CP_METAS[tag].name);
	obj->tag = tag;
	obj->info.cp_module.name_index = R_BIN_JAVA_USHORT (buffer, 1);
	return obj;
}

static ut64 module_cp_calc_size(RBinJavaCPTypeObj *obj) {
	return 3; // 1 tag + 2 name_index
}

static RBinJavaCPTypeObj *package_cp_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz) {
	ut8 tag = buffer[0];
	int quick_check = r_bin_java_quick_check (R_BIN_JAVA_CP_PACKAGE, tag, sz, "RBinJavaCPTypePackage");
	if (quick_check > 0) {
		return NULL;
	}
	RBinJavaCPTypeObj *obj = R_NEW0 (RBinJavaCPTypeObj);
	...
	obj->name = strdup ((const char *)R_BIN_JAVA_CP_METAS[tag].name);
	obj->tag = tag;
	obj->info.cp_package.name_index = R_BIN_JAVA_USHORT (buffer, 1);
	return obj;
}
...
static void cp_default_free(void /* RBinJavaCPTypeObj*/ *o) {
	RBinJavaCPTypeObj *obj = o;
	if (obj) {
		free (obj->metas);
		free (obj->name);
...
static void utf8_info_free(void /* RBinJavaCPTypeObj*/ *o) {
	RBinJavaCPTypeObj *obj = o;
	if (obj) {
		free (obj->name);
		free (obj->metas);
...
static void r_bin_java_print_attr_summary(RBinJavaAttrInfo *attr) {
	...
	((const RBinJavaAttrTypeInfo *)attr->metas->type_info)->print_summary (attr);
}
...
static char *r_bin_java_print_methodhandle_cp_stringify(RBinJavaCPTypeObj *obj) {
	...
	return r_str_newf ("%d.0x%04" PFMT64x ".%s.%s.%d",
		...
		obj->file_offset + obj->loadaddr,
		((RBinJavaCPTypeMetas *)obj->metas->type_info)->name,
		R_BIN_JAVA_REF_METAS[ref_kind].name,
		obj->info.cp_method_handle.reference_index);
}
...
static RBinJavaElementValuePair *r_bin_java_element_pair_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	evp->name = r_bin_java_get_utf8_from_bin_cp_list (bin, evp->element_name_idx);
	if (!evp->name) {
		R_LOG_WARN ("ElementValue Name is invalid");
...
static RBinJavaAttrInfo *r_bin_java_annotation_default_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut64 offset = 0;
	if (sz < 8) {
		return NULL;
	}
...
static RBinJavaElementValue *r_bin_java_element_value_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	case R_BIN_JAVA_EV_TAG_STRING:
		...
		element_value->value.const_value.const_value_cp_obj = r_bin_java_clone_cp_idx (bin, element_value->value.const_value.const_value_idx);
		break;
	case R_BIN_JAVA_EV_TAG_ENUM:
		// (ut16) read and set enum_const_value.type_name_idx
		...
		element_value->value.enum_const_value.const_name_cp_obj = r_bin_java_clone_cp_idx (bin, element_value->value.enum_const_value.const_name_idx);
		// look-up, deep copy, and set enum_const_value.type_name_cp_obj
		element_value->value.enum_const_value.type_name_cp_obj = r_bin_java_clone_cp_idx (bin, element_value->value.enum_const_value.type_name_idx);
		break;
	case R_BIN_JAVA_EV_TAG_CLASS:
		// (ut16) read and set class_value.class_info_idx
		...
		element_value->value.class_value.class_info_cp_obj = r_bin_java_clone_cp_idx (bin, element_value->value.class_value.class_info_idx);
		break;
	case R_BIN_JAVA_EV_TAG_ARRAY:
		// (ut16) read and set array_value.num_values
		...
		// Validate num_values to prevent excessive looping and malformed input
		if (element_value->value.array_value.num_values > (sz - offset)) {
			R_LOG_ERROR ("Invalid array value count: %u", (unsigned)element_value->value.array_value.num_values);
			r_bin_java_element_value_free (element_value);
			return NULL;
		}
...
static RBinJavaBootStrapArgument *r_bin_java_bootstrap_method_argument_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	offset += 2;
	bsm_arg->argument_info_cp_obj = r_bin_java_clone_cp_idx (bin, bsm_arg->argument_info_idx);
	bsm_arg->size = offset;
	return bsm_arg;
}
...
static RBinJavaBootStrapMethod *r_bin_java_bootstrap_method_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	...
	// Validate num_bootstrap_arguments to prevent excessive looping and buffer overread
	{
		ut64 remaining = (sz > offset)? (sz - offset): 0;
		ut64 max_bootstrap_arguments = remaining / 2;
		if (bsm->num_bootstrap_arguments > max_bootstrap_arguments) {
			R_LOG_ERROR ("Invalid bootstrap arguments count: %u", bsm->num_bootstrap_arguments);
	...
	for (i = 0; i < bsm->num_bootstrap_arguments; i++) {
		...
		bsm_arg = r_bin_java_bootstrap_method_argument_new (bin, buffer + offset, sz - offset, buf_offset + offset);
		if (bsm_arg) {
			offset += bsm_arg->size;
...
static RBinJavaAttrInfo *r_bin_java_bootstrap_methods_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut32 i = 0;
	RBinJavaBootStrapMethod *bsm = NULL;
	ut64 offset = 0;
	RBinJavaAttrInfo *attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
...
static RBinJavaAttrInfo *r_bin_java_rtip_annotations_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut32 i = 0;
	RBinJavaAttrInfo *attr = NULL;
	ut64 offset = 0;
	...
	if (attr) {
		...
		for (i = 0; i < attr->info.rtip_annotations_attr.num_parameters; i++) {
			...
			RBinJavaAnnotationsArray *annotation_array = r_bin_java_annotation_array_new (bin,
				buffer + offset,
				sz - offset,
				buf_offset + offset);
			if (annotation_array) {
...
static RBinJavaAttrInfo *r_bin_java_rtvp_annotations_attr_new(RBinJavaObj *bin, ut8 *buffer, ut64 sz, ut64 buf_offset) {
	ut32 i = 0;
	RBinJavaAttrInfo *attr = NULL;
	ut64 offset = 0;
	attr = r_bin_java_default_attr_new (bin, buffer, sz, buf_offset);
...
// TODO: return char * instead of printing
static void r_bin_java_print_rtip_annotations_attr_summary(RBinJavaAttrInfo *attr) {
	RBinJavaAnnotationsArray *annotation_array = NULL;
	RListIter *iter = NULL, *iter_tmp = NULL;
	if (attr && attr->type == R_BIN_JAVA_ATTR_TYPE_RUNTIME_INVISIBLE_PARAMETER_ANNOTATION_ATTR) {
...
static RBinJavaCPTypeObj *r_bin_java_find_cp_name_and_type_info(RBinJavaObj *bin, ut16 name_idx, ut16 descriptor_idx) {
	...
	r_list_foreach_safe (bin->cp_list, iter, iter_tmp, obj) {
		if (obj && obj->tag == R_BIN_JAVA_CP_NAMEANDTYPE) {
			// eprintf ("RBinJavaCPTypeNameAndType has name_idx: %d and descriptor_idx: %d\n",
...
R_API char *r_bin_java_resolve_cp_idx_type(RBinJavaObj *BIN_OBJ, int idx) {
	...
	RBinJavaCPTypeObj *item = (RBinJavaCPTypeObj *)r_bin_java_get_item_from_bin_cp_list (BIN_OBJ, idx);
	if (item) {
		return strdup (((RBinJavaCPTypeMetas *)item->metas->type_info)->name);
	}
...
static char *r_bin_java_resolve(RBinJavaObj *BIN_OBJ, int idx, ut8 spacy) {
	...
	*str = NULL;
	...
	if (item) {
		...
		R_LOG_DEBUG ("java_resolve Resolved: (%d) %s", idx, cp_name);
	} else {
		return r_str_newf ("(%d) INVALID CP_OBJ", idx);
	}
	if (!strcmp (cp_name, "Class")) {
		RBinJavaCPTypeObj *item2 = (RBinJavaCPTypeObj *)r_bin_java_get_item_from_bin_cp_list (BIN_OBJ, idx);
		char *name_str = r_bin_java_get_item_name_from_bin_cp_list (BIN_OBJ, item2);
		char *desc_str = r_bin_java_get_item_desc_from_bin_cp_list (BIN_OBJ, item2);
		str = r_str_newf ("%s%s%s", r_str_get (name_str), spacy? " ": "", r_str_get (desc_str));
		free (name_str);
		free (desc_str);
		...
		str = r_str_newf ("%s/%s%s%s", class_str, name_str, spacy? " ": "", desc_str);
		if (class_str != empty) {
			free (class_str);
		}
		if (name_str != empty) {
		...
		char *name_str = r_bin_java_get_item_name_from_bin_cp_list (BIN_OBJ, item);
		char *desc_str = r_bin_java_get_item_desc_from_bin_cp_list (BIN_OBJ, item);
		str = r_str_newf ("%s%s%s", r_str_get (name_str), spacy? " ": "", r_str_get (desc_str));
		free (name_str);
		free (desc_str);
	} else {
...
R_API RList *r_bin_java_get_method_definitions(RBinJavaObj *bin) {
	...
	r_list_foreach (bin->methods_list, iter, fm_type) {
		r_list_append (the_list, r_bin_java_unmangle (fm_type->flags_str, fm_type->name, fm_type->descriptor));
	}
	return the_list;
}
	...
	if (!strcmp (cp_name, "Class")) {
		...
		if (memory_alloc) {
			...
			if (str) {
				...
				out = r_base64_encode_dyn ((const ut8 *)str, strlen (str));
				free (str);
				str = out;
			}
		}
...
R_API void deinit_java_type_null(void) {
...
R_API void copy_type_info_to_stack_frame_list(RList *type_list, RList *sf_list) {
	RListIter *iter, *iter_tmp;
...
R_API void copy_type_info_to_stack_frame_list_up_to_idx(RList *type_list, RList *sf_list, ut64 idx) {
	RListIter *iter, *iter_tmp;
	RBinJavaVerificationObj *ver_obj, *new_ver_obj;
	ut32 pos = 0;
	if (!type_list || !sf_list) {
